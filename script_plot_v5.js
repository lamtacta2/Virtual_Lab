...
.update({
  ...
  document.getElementById("Run").onclick = function Plot_MP(){
    readFom();
    firebase
        .database()
        .ref("Input")
        ...
        (async() => {
          ...
          document.getElementById("Output").innerHTML = workbook2.Sheets.Sheet1["A2"].v
         ...
         function data_update(k){
          for(let i = 2; i < k; i++){
              ...
              data4[i-2] = workbook1.Sheets.Sheet1[locale1].v.slice(1,workbook1.Sheets.Sheet1[locale1].v.length-1);
              data5[i-2] = workbook1.Sheets.Sheet1[locale2].v;
              labelsa[i-2] = i-2;   
          }}
